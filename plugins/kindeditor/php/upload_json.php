...
if (empty ( $_FILES ) === false) {
	...
	$ground_info = getimagesize ( $file_path );
	$ground_w = $ground_info [0]; // 取得背景图片的宽
	$ground_h = $ground_info [1]; // 取得背景图片的高
	$maxWidth = 800;
	if (($dir_name == 'image') && ($ground_w > $maxWidth)) {
		$imageWidth = $maxWidth;
		$imageHeight = round ( $ground_h * $imageWidth / $ground_w );
		thumb ( $file_path, $file_path, $imageWidth, $imageHeight );
	}
	...
	/* 水印配置开始 */
	$water_mark = ($dir_name == 'image' ? 1 : 0); // 1为加水印, 其它为不加
	$ground_info = getimagesize ( $file_path );
	if ($ground_info[2]==6) {
		$water_mark = 0;
	}
	$water_pos = 1; // 水印位置，10种状态【0为随机，1为顶端居左，2为顶端居中，3为顶端居右；4为中部居左，5为中部居中，6为中部居右；7为底端居左，8为底端居中，9为底端居】
	$water_img = $_SERVER ['DOCUMENT_ROOT'] . '/yanding/images/logo1.png'; // 水印图片,默认填写空,请将图片上传至网站根目录的upfiles下,例: water.gif
...
function imageWaterMark($groundImage, $waterPos = 0, $waterImage = '', $alpha = 80, $waterText = '', $water_fontfile, $textFont = 9, $textColor = '#FF0000') {
	...
	if ($isWaterImage) { // 图片水印
	                   ...
	                   // 生成混合图像
		imagecopymerge_alpha ( $ground_im, $water_im, $posX, $posY, 0, 0, $water_w, $water_h, $alpha );
	} else { // 文字水印
...
// 用imagecopymerge_alpha代替imagecopymerge，可以使用背景透明的水印图片
