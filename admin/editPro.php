...
if (!$big_cates) {
	alertMes("没有相应的商品大类，请先添加大分类", "addBigCate.php");
}
$small_cates = getAllSmallCate();
if(!$small_cates){
	alertMes("没有相应小类，请先添加小分类!!", "addSmallCate.php");
}
...
<script>
   ...
   　　 	// 大类变化时 处理小类
        function changeCate(){
        	$("#chooseSmallCate").attr("value","-1");
            $("#chooseSmallCate").html("请选择小类");
            var sCate = $("#small_cId");
...
<table width="70%"  border="1" cellpadding="5" cellspacing="0" bgcolor="#cccccc">
	<tr>
		<td align="right">商品中文名称</td>
		<td><input type="text" name="pName"  value="<?php echo $proInfo['pName']?>" required/></td>
	</tr>
	<tr>
		<td align="right">商品统称</td>
		<td><input type="text" name="pNum"  value="<?php echo $proInfo['pNum']?>" required/></td>
	</tr>
	<tr>
		<td align="right">关联商品统称</td>
		<td><input type="text" name="rePNum"  value="<?php echo $proInfo['rePNum']?>"/>多个统称以逗号(英文输入)隔开</td>
	</tr>
	<tr>
		...
		<td>
			...
			<div class="modItem">
				...
				<input class="partNum" type="text" name="partNum[]" value="<?php echo $_model['partNum'];?>" required/>
	...
	<tr>
		...
		<td>
			<select name="big_cId" id="big_cId" onchange="changeCate();" min="1">
				<option id="chooseBigCate" value="<?php echo $proInfo['big_cId'];?>"><?php echo $proInfo['big_cName'];?></option>
			</select>
		</td>
	</tr>
	<tr>
	...
	　　　　　　<select name="small_cId" id="small_cId" style="float: left;" min="1">
				<option id="chooseSmallCate" value="<?php echo $proInfo['small_cId'];?>"><?php echo $proInfo['small_cName'];?></option>
			</select>
　　　　</td>
	</tr>
	<tr>
		...
		<select name="bra_cId">
			<?php foreach($bra_cates as $bra_cate):?>
				<option value="<?php echo $bra_cate['id'];?>"<?php echo $bra_cate['id']==$proInfo['bra_cId']?"selected='selected'":null;?>><?php echo $bra_cate['bra_cName'];?></option>
			<?php endforeach;?>
		</select>
		</td>
	</tr>
	<tr>
		...
		<td>
			<select name="pShow">
				<?php for($i=0;$i<=2;$i++):?>
					<option value="<?php echo $i;?>" <?php echo $i==$proInfo['pShow']?"selected='selected'":null; ?>><?php echo $i;?></option>
				<?php endfor;?>
			</select>
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			<?php for($i=0;$i<=16;$i++):?>
				<option value="<?php echo $i;?>" <?php echo $i==$proInfo['pRecommend']?"selected='selected'":null; ?>><?php echo $i;?></option>
			<?php endfor;?>
			</select>
		</td>
	</tr>
	<tr>
		...
		<td>
			<textarea name="pAbstract" style="width:99%;height:150px;" maxlength="100" placeholder="最多输入100字"><?php echo $proInfo['pAbstract'];?></textarea>
		</td>
	</tr>
	<tr>
